...
fn get_latest_evaluation(
    ...
    let mut evals = get_url::<JobsetEvals>(&format!("/jobset/{project_name}/{jobset_name}/evals"))?.evals;
...
fn get_failing_builds(evaluation: &Evaluation) -> impl Iterator<Item = Build> + '_ {
    evaluation.builds
        .iter()
...
fn main() {
    let evaluation =
        get_latest_evaluation("nixpkgs", "trunk").unwrap();
    let failing_build = get_failing_builds(&evaluation).next().unwrap();
    println!("{:#?} has failed", failing_build.nixname);
}
